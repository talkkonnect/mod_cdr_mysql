...
static switch_status_t insert_cdr(const char *values)
{
	...
	if (mysql_affected_rows(globals.db_connection) == -1) {
	...
	if (mysql_affected_rows(globals.db_connection) == 0) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "INSERT command failed No Rows Inserted: %s", mysql_error(globals.db_connection));
		goto error;
	}
